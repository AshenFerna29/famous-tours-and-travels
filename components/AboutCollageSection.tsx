...
export default function AboutCollageSection({
  ...
  const safe = (i: number) => images[i] || { src: "/images/placeholder.png", alt: "" };
        ...
        <div className="md:col-span-7">
          ...
          {titleItalic && (
            <div className="flex items-center gap-4 mt-1 mb-4">
              <p className="italic text-2xl md:text-[28px] font-medium">{titleItalic}</p>
...
<div className="md:col-span-5">
  <div className="grid grid-cols-2 gap-4 content-start max-w-[520px] ml-auto">
    ...
    <div className="relative h-32 md:h-36 overflow-hidden rounded-xl shadow-md">
      <Image fill src={safe(0).src} alt={safe(0).alt || ""} className="object-cover" />
    </div>
    <div className="relative h-32 md:h-36 overflow-hidden rounded-xl shadow-md">
      <Image fill src={safe(1).src} alt={safe(1).alt || ""} className="object-cover" />
    </div>

    {/* Row 2: one taller portrait + one small landscape */}
    <div className="relative h-44 md:h-56 overflow-hidden rounded-xl shadow-md">
      <Image fill src={safe(2).src} alt={safe(2).alt || ""} className="object-cover" />
    </div>
    <div className="relative h-32 md:h-36 overflow-hidden rounded-xl shadow-md">
      <Image fill src={safe(3).src} alt={safe(3).alt || ""} className="object-cover" />
    </div>
  </div>
</div>
